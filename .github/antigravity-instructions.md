...
- **Usa `interface` para:**
  - Puertos (Contratos que los adaptadores deben cumplir).
...
- **Manejo de RxJS:**
  - Evita anidar subscripciones (Callback Hell). Usa operadores como `switchMap`, `mergeMap`, etc.
...
# Reglas de Interacción para Tizen OS
Mi app es una app para TV Samsung con Tizen, por lo que el foco es la navegación con el control remoto, y no con mouse o teclado común.
- El foco principal de la interacción en pantalla es la navegación con los botones direccionales (Flechas Up/Down/Left/Right) y la confirmación con el botón de OK/Enter del control.
- Por lo tanto, no uses dependencias de "Tab" o "Enter de teclado regular" en tus sugerencias y estructuras de navegación, sino enfocándote en el control del Spatial Navigation con las flechas y acción en OK.
- Considera el teclado en pantalla (OSK - On Screen Keyboard) cuando propongas un `<input>`.
